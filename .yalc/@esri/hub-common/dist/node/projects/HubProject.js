 ...
 * Hub Project Class
 */
class HubProject extends HubItemEntity_1.HubItemEntity {
    /**
    ...
    constructor(project, context) {
        ...
        this._catalog = search_1.Catalog.fromJson(project.catalog, this.context);
    }
    /**
    ...
    static async fetch(identifier, context) {
        ...
        try {
            const project = await fetch_1.fetchProject(identifier, context.requestOptions);
    ...
    static async getEditorConfig(i18nScope, type, options = []) {
        const { getHubProjectEditorConfig } = await Promise.resolve().then(() => require("./edit"));
    ...
    static applyDefaults(partialProject, context) {
        ...
        const pojo = Object.assign(Object.assign({}, defaults_1.DEFAULT_PROJECT), partialProject);
        return pojo;
    }
    /**
    ...
    async save() {
        ...
        const { createProject, updateProject } = await Promise.resolve().then(() => require("./edit"));
        if (this.entity.id) {
            // update it
    ...
    async delete() {
        ...
        const { deleteProject } = await Promise.resolve().then(() => require("./edit"));
        // Delegate to module fn
