 ...
 * @returns
 */
async function deepContains(identifier, hierarchy, context) {
    ...
    await utils_1.asyncForEach(hierarchy, async (catalogInfo, idx) => {
        const currentIdentifier = idsToCheck[idx];
        // create a catalog instance
        let catalog;
        if (catalogInfo.catalog) {
            // create instance
            catalog = search_1.Catalog.fromJson(catalogInfo.catalog, context);
        }
        else {
            // otherwise, fetch the catalog and check it
            catalog = await search_1.Catalog.init(catalogInfo.id, context);
        }
