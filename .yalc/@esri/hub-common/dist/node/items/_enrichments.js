...
async function parseMetadataXml(metadataXml) {
    ...
    const { parse } = await Promise.resolve().then(() => require("fast-xml-parser"));
...
async function fetchContentMetadata(id, requestOptions) {
    try {
        const metadataXml = await arcgis_rest_portal_2.getItemMetadata(id, requestOptions);
...
const enrichOrg = (input) => {
    ...
    const orgId = _internal_1.getItemOrgId(data.item, data.ownerUser);
    // Only fetch the org if an explicit orgId is present
    const orgPromise = orgId
        ? org_1.fetchOrg(orgId, requestOptions)
...
const enrichServer = (input) => {
    ...
    const url = arcgis_rest_feature_layer_1.parseServiceUrl(data.item.url);
